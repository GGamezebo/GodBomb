...
static func _side_ellipsis_bbcode(dim_size: int) -> String:
	return "[font_size=%d][color=#FFFFFF]%s[/color][/font_size]" % [dim_size, PATTERN_SIDE_ELLIPSIS]


static func _measure_side_ellipsis_width(dim_size: int) -> float:
	return _measure_text_width(PATTERN_SIDE_ELLIPSIS, dim_size)
...
static func _measure_pattern_width(syllable: String, condition: int, hi_size: int, dim_size: int) -> float:
	if uses_exclusion_rules():
		match condition:
			...
			Type.ANYWHERE:
				return (
					_measure_side_ellipsis_width(dim_size)
