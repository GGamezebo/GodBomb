...
func _process(delta: float) -> void:
	if _alert_active and bomb_sprite:
		...
		bomb_sprite.scale = Vector2.ONE * (BASE_BOMB_SCALE * 0.625 + pulse)
...
func _play_ready() -> void:
	...
	bomb_sprite.modulate = Color.WHITE
	bomb_sprite.scale = Vector2.ONE * BASE_BOMB_SCALE
	_tween = create_tween()
	_tween.tween_property(bomb_sprite, "scale", Vector2.ONE * (BASE_BOMB_SCALE * 1.04), 0.35).set_trans(Tween.TRANS_SINE)
...
func _play_comes() -> void:
	...
	_kill_tween()
	bomb_sprite.visible = true
	bomb_sprite.scale = Vector2.ONE * (BASE_BOMB_SCALE * 0.4)
	bomb_sprite.modulate = Color(1, 1, 1, 0.2)
	_tween = create_tween().set_parallel(true)
	_tween.tween_property(bomb_sprite, "scale", Vector2.ONE * (BASE_BOMB_SCALE * 1.1), 0.45).set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
...
func _play_boom() -> void:
	...
	_kill_tween()
	_tween = create_tween().set_parallel(true)
	_tween.tween_property(bomb_sprite, "scale", Vector2.ONE * (BASE_BOMB_SCALE * 1.8), 0.18).set_trans(Tween.TRANS_EXPO).set_ease(Tween.EASE_OUT)
	...
	_tween.parallel().tween_property(bomb_sprite, "scale", Vector2.ONE * (BASE_BOMB_SCALE * 2.4), 0.25)
